...
def AI_loop():
    ...
    loops = {1: AI_move, 2: AI_atk, 3: AI_def}
    ai.switchLoop(loops[typ])
...
def runBot(gene, num, port, typ):
    try:
        ...
        ai.start(AI_loop, ["-name", f"test_bot_{num}", "-join", "localhost", "-port", str(port)])

        if typ == 1:
            out_dir = os.path.join("botData", "move")
        elif typ == 2:
            out_dir = os.path.join("botData", "atk")
        else:
            out_dir = os.path.join("botData", "def")

        os.makedirs(out_dir, exist_ok=True)
        out_path = os.path.join(out_dir, f"bot_{num}.txt")
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description="XPilot bot (move/atk/def) parameterized by a genome.")
    parser.add_argument("--gene", required=False, help="Genome as bitstring (e.g. 0101...) or Python list (legacy).")
    parser.add_argument("--num", required=False, type=int, help="Bot id number used for output filename.")
    parser.add_argument("--port", required=False, type=int, help="XPilot server port.")
    parser.add_argument("--typ", required=False, help="1/2/3 or move/atk/def.")
    parser.add_argument("--limit", required=False, type=int, help="Frame limit before quitting.")
    args, unknown = parser.parse_known_args()

    # Legacy positional support (original script expected argv[2..6]).
    if args.gene is None and len(sys.argv) >= 7:
        args.gene = sys.argv[2]
        args.num = int(sys.argv[3])
        args.port = int(sys.argv[4])
        args.typ = sys.argv[5]
        args.limit = int(sys.argv[6])

    if args.gene is None or args.num is None or args.port is None or args.typ is None or args.limit is None:
        raise SystemExit("Missing required arguments. Use --gene/--num/--port/--typ/--limit (or legacy positional args).")

    typ_raw = str(args.typ).strip().lower()
    if typ_raw in {"1", "move"}:
        typ = 1
    elif typ_raw in {"2", "atk", "attack"}:
        typ = 2
    elif typ_raw in {"3", "def", "defense"}:
        typ = 3
    else:
        raise SystemExit(f"Invalid --typ '{args.typ}'. Expected 1/2/3 or move/atk/def.")

    limit = int(args.limit)

    gene_raw = str(args.gene).strip()
    if gene_raw and all(ch in "01" for ch in gene_raw):
        gene_bits = list(gene_raw)
    else:
        # Legacy: allow a Python literal like "['0','1',...]" or "[0,1,...]".
        gene_bits = ast.literal_eval(gene_raw)

    setUp(gene_bits, typ)
    runBot(gene_bits, args.num, args.port, typ)
